...
-(void)showInput:(NSString *)text {
    ...
    _textView.text = text;
    _placeholderLab.hidden = text.length > 0;
...
-(void)hide{
    ...
    if([self.delegate respondsToSelector:@selector(xhInputViewWillHide:text:)]){
        [self.delegate xhInputViewWillHide:self text:_textView.text];
    }
...
-(void)setSendButtonTitle:(NSString *)sendButtonTitle{
    ...
    [_sendButton setTitleColor:kTitleColor forState:UIControlStateNormal];
}
